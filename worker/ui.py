...
class TerminalUI:
    ...
    def run(self):
        """
        Run the UI. If the bridge configuration has 
        terminal_ui_enabled set to False, then run plain mode,
        printing status and log output to the console.
        """
        if not getattr(self.bridge_data, "terminal_ui_enabled", True):
            self.run_plain_mode()
        else:
    ...
    def run_plain_mode(self):
        """
        Run in plain mode without clearing the console.
        The status is printed once, and then new log entries are appended continuously.
        """
        # Print status information once.
        print("=== Worker Status ===")
        print(f"Worker Name: {self.worker_name}")
        print(f"CPU Usage: {self.get_cpu_usage()}")
        print(f"Free RAM: {self.get_free_ram()}")
        print("")
        print("=== Logs ===")
        
        # Index to track how many log lines have been printed.
        last_log_count = 0
        try:
            while True:
                # Update our internal log queue.
                self.load_log()
                logs = list(self.output.deque)
                # Print new log lines if there are any.
                if len(logs) > last_log_count:
                    for line in logs[last_log_count:]:
                        print(line.replace(TerminalUI.DELIM, " | "))
